...
export async function POST(request: NextRequest) {
  ...
  const entry = {
    ...
    link,
    headline: input?.headline?.trim().slice(0, 180) || previous?.headline || "Running a little further today.",
    category: input?.category?.trim().slice(0, 40) || previous?.category || "Running",
